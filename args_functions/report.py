# Import
from settings import CSV_SALE, console
...
def report(args):
    if args.type == "revenue" and args.item.lower() == "all":
        ...
        console.print(
            f"[bright_cyan]Per [bold]{args.period}[/bold] the totall revenue of all sold products is [bold]€ {display}[/bold][bright_cyan]")
        ...
        console.print(
            f"[bright_cyan]Per [bold]{args.period}[/bold] the totall revenue of [bold]{args.item.lower()}[/bold] is € [bold]{display}[/bold][bright_cyan]")
        ...
        console.print(
            f"[bright_cyan]Per [bold]{args.period}[/bold] the totall profit of all sold products is € [bold]{display}[/bold][bright_cyan]")
    elif args.type == "profit" and args.item.lower() != "all":
